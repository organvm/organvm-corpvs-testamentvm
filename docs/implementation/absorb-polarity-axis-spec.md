...
**Infrastructure sentinel.** Structural-only signal: `functional_class == "INFRASTRUCTURE"`. The registry stores `functional_class` as an **uppercase** enum, so the implementation must compare case-insensitively (e.g. `str(fc).upper() == "INFRASTRUCTURE"`) — otherwise scaffolding repos (`.github`, `schema-definitions`, `organvm-engine`) miss the sentinel and get force-classified, violating the nullable-infrastructure requirement. When it fires, the classifier proposes `N/A — infrastructure` and must **not** force a creative polarity (the GH#97 §4.4 refinement). This is the one case where structural signal overrides any weak textual lean.
