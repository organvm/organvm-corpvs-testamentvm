...
> **Canonical-home note (RESOLVED 2026-06-10).** The resolver now lives **here**, at `scripts/dependabot-tier-classify` — public, auditable next to the rules it enforces, and CI-testable via `scripts/dependabot-tier-classify --self-test` (validates the Tier-1-preserving invariants against the live `repo-registry.json`). The `~/.local/bin/dependabot-tier-classify` entry on `PATH` (deployed by `domus` chezmoi) is a **thin wrapper** that execs this implementation at `${ORGANVM_CORPUS_DIR:-$HOME/Code/organvm/organvm-corpvs-testamentvm}/scripts/dependabot-tier-classify`, so there is a single source of truth and every agent runtime (Claude, opencode, gemini, codex) invokes the same code.
