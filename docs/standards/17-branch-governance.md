...
> **Canonical-home note (RESOLVED 2026-06-10).** The resolver now lives **here**, at `scripts/dependabot-tier-classify` — public, auditable next to the rules it enforces, and CI-testable via `scripts/dependabot-tier-classify --self-test` (validates the Tier-1-preserving invariants against the live `repo-registry.json`). **Sequencing:** on merge of this branch the script lands on `main`; at that point the `~/.local/bin/dependabot-tier-classify` entry on `PATH` (deployed by `domus` chezmoi) is converted to a **thin wrapper** that execs `${ORGANVM_CORPUS_DIR:-$HOME/Code/organvm/organvm-corpvs-testamentvm}/scripts/dependabot-tier-classify` — establishing a single source of truth for every agent runtime (Claude, opencode, gemini, codex). **Until merge** the `~/.local/bin/` entry carries a byte-identical copy, so the routines keep working; the wrapper swap is gated on this script being present on `main` (doing it earlier would point the wrapper at a file that does not yet exist there). Post-merge follow-up.
