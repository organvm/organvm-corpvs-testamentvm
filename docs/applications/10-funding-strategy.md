# Funding Strategy — Benefits-Cliff-Aware, Deadline-Sequenced

**Created:** 2026-02-17
**Purpose:** Master funding strategy merging emergency-help research with application planning
**Status:** ACTIVE — expanded with exhaustive research (200+ opportunities cataloged)
**Companion:** [04-application-tracker.md](./04-application-tracker.md) (status tracking), [09-qualification-assessment.md](./09-qualification-assessment.md) (fit scores), [11-funding-research-exhaustive.md](./11-funding-research-exhaustive.md) (master reference)
**Sources:** FILE-059 (benefits cliff), FILE-060 (12-month transition), FILE-058 (market intelligence), FUNDING_ACTION_SEQUENCE.md, 5-domain exhaustive research (2026-02-17)

---

## Benefits Cliff Summary

> **The safety net is an asset, not a constraint to escape.**
> Understanding the cliff mechanics lets you pursue funding without losing essential benefits.

**Current situation:** Single adult, NYC, ~$12K adjunct base, receiving SNAP + Medicaid + Fair Fares.

| Program | Threshold | Grant Treatment | Risk Level |
|---------|-----------|-----------------|------------|
| **SNAP** | $20,352/yr (130% FPL) | **Lump-sum grants EXCLUDED entirely** (BBCE) | **SAFE** |
| **Medicaid** | $21,597/yr (138% FPL) | Taxable grants count toward MAGI | Moderate above ~$9.6K in grants |
| **Essential Plan** | $39,125/yr (250% FPL) | Same as Medicaid, but $0 premium bridge | Low — replaces Medicaid at no cost |
| **Fair Fares** | $22,692/yr (145% FPL) | Counts toward annual income | **Hard cliff** — loss costs ~$700-800/yr |
| **Section 8/NYCHA** | $56,700 (50% AMI) | One-time non-recurring excluded | **SAFE** |

**Safe grant-stacking ceiling:** ~$9K–$12K/year in one-time grants (on $12K adjunct base) preserves ALL benefits. With Schedule C deductions (art supplies, equipment, portfolio hosting), gross ceiling rises to ~$11K–$14K.

**If you exceed Medicaid threshold:** NYS Essential Plan covers you at **$0 premium** up to $39,125/year. The practical health coverage cliff is NOT at $21,597 — it's at $39,125.

**Key rule:** Call **NYLAG (212-613-5000)** before accepting any single grant >$5K. They provide free combined legal + financial counseling for exactly this situation.

---

## Four Identity Positions

Different funders respond to different framings. Same body of work, four lenses:

### 1. Educator
- 100+ courses, 2,000+ students, 8+ institutions, 11 years
- **Targets:** Spencer Foundation, NEH, state arts councils, university partnerships
- **Narrative:** "Teaching complex systems to diverse audiences IS my creative practice"

### 2. Multimedia Artist / Systems Artist
- 150-repo eight-organ system as living creative work, 42 essays, MFA
- **Targets:** Creative Capital, Artadia, FCA, NYSCA/NYFA, residencies
- **Narrative:** "I build creative systems — the governance IS the artwork"

### 3. AI Systems Architect / Creative Technologist
- Multi-agent orchestration, autonomous governance, 2,349 tests
- AI orchestration market: $47–52B projected by 2030
- **Targets:** Consulting clients, selective tech roles, accelerators
- **Narrative:** "Production-grade AI orchestration with creative-artistic applications"

### 4. Community-Focused Practitioner
- LGBTQ+, housing-precarious, recovery-informed practice
- **Targets:** Queer|Art, Fire Island, TJFP, Lambda Literary, identity-specific grants
- **Narrative:** "Building creative infrastructure from lived experience of precarity"

---

## Deadline-Sequenced Targets

All deadlines verified as of 2026-02-17. Ordered chronologically.

### URGENT — Within 2 Weeks

| Opportunity | Type | Amount | Deadline | Fit | Action |
|-------------|------|--------|----------|-----|--------|
| **Artadia NYC Awards** | Unrestricted grant | **$15,000** | **Mar 1** | 8/10 | Begin application NOW (Submittable) |
| **Doris Duke / Mozilla AMT Lab** | Grant | **Up to $150,000** | **Mar 2** | 7/10 | Performing arts + digital tools. Highest single amount. |
| **Prix Ars Electronica** | International prize | **EUR 10,000** | **Mar 4** | 8/10 | Free to submit. Governance-as-art narrative |
| **S+T+ARTS Prize** | International prize | **EUR 20,000** | **Mar 4** | 8/10 | Science+Technology+Arts nexus |
| **NEH Summer Programs** | Stipended program | Stipend | **Mar 6, 11:59pm PT** | 5/10 | Research eligibility + submit |
| **PEN America Writers Aid** | Emergency | **Up to $3,500** | Rolling | **9/10** | **APPLY NOW** — highest-fit emergency fund |

### This Month (Feb 17 – Mar 18)

| Opportunity | Type | Amount | Deadline | Fit | Action |
|-------------|------|--------|----------|-----|--------|
| **Bread Loaf Conference** | Writing scholarship | **$4,440** value | **Mar 15** | 7/10 | Nonfiction track, MFA + 42 essays |
| **Google Creative Fellowship** | Paid fellowship (6 mo) | Stipend (~$50K+) | **Mar 18, 5pm PST** | 7/10 | Submit (materials ready) |
| **Google Creative Lab Five** | Paid residency (1 yr) | Salary | Open (no close date) | 8/10 | Submit ASAP |
| **Noema Magazine** | Writing income | **$1/word** (~$3K/essay) | Rolling | — | Pitch this month |
| **Gay & Lesbian Review** | Writing income | **$250/feature** | Rolling | 9/10 | Submit this month |

### Next 1–3 Months

| Opportunity | Type | Amount | Deadline | Fit | Action |
|-------------|------|--------|----------|-----|--------|
| **Fire Island Artist Residency** | Residency | Stipend + housing | **Apr 1, 11:59pm** | 7/10 | Begin via SlideRoom (LGBTQ+) |
| **NLnet NGI Zero Commons** | Grant | **EUR 5K–50K+** | **Apr 1** | 7/10 | Open-source infrastructure |
| **Headlands Center** | Residency | **Fully funded** + $1K/mo | **Apr 1–Jun 1** | **9/10** | Premier fully-funded residency |
| **Creative Capital 2027** | Grant | **Up to $50,000** + services | **Apr 2, 3pm ET** | **9/10** | Portal opens Mar 2 — HIGHEST PRIORITY |
| **ZKM Rauschenberg** | Residency | Studios, 3 months | **Apr 12** | 8/10 | Premier media art institution |
| **WFF Housing Stability** | Grant | **$30,000/3yr** | **Apr 14, 5pm ET** | **9/10** | Highest-value NYC opportunity |
| **Rauschenberg Cycle 36** | Emergency grant | **Up to $5,000** | **Apr 14–May 12** | 6/10 | Opens Apr 14, quick application |
| **Spencer Foundation Small** | Research grant | **Up to $50,000** | **Apr 15, noon CT** | 6/10 | Education research angle |
| **Whiting Nonfiction** | Grant | **$40,000** | **~Apr** | 8/10 | 404K-word corpus as book pitch |
| **Processing Foundation** | Fellowship | **$10,000** | **~Apr-May** | 6/10 | Monitor for 2026 announcement |
| **Warhol Arts Writers** | Grant | **$15K–$50K** | **Opens May 1** | 7/10 | Writing about contemporary art |
| **Eyebeam** | Residency | **$4,000** + studio | **Spring 2026 (TBA)** | 7/10 | Check Submittable portal |

### Dropped Targets

| Target | Old Fit | Reason |
|--------|---------|--------|
| Knight Foundation Art+Tech | 1/10 | Geographically restricted to Akron/Detroit/Miami |
| Mellon Foundation Arts & Culture | 0/10 | Invitation-only — cannot apply |
| NEA GAP (direct) | 1/10 | Requires 501(c)(3) — revisit after Fractured Atlas |

---

## Rolling & Infrastructure — No Deadlines

These can be activated immediately and provide ongoing support:

| Opportunity | Type | Amount | Frequency | Action |
|-------------|------|--------|-----------|--------|
| **GitHub Sponsors** | Sponsorship | Variable | Ongoing | **Activate NOW** (~15 min, zero fee) |
| **Fractured Atlas** | Fiscal sponsorship | Infrastructure | Rolling | **Apply this week** ($10/mo, unlocks org-only grants like NEA) |
| **FCA Emergency Grants** | Emergency | $200–$3,000 | Monthly review | Apply (requires confirmed public presentation) |
| **Max's Kansas City Project** | Emergency | $500–$1,000 | Rolling | NYS resident artists, housing/legal/medical |
| **Artists' Fellowship Inc.** | Emergency | Varies | Annual | NYC-based artists |

---

## Career-Building (3–12 Months)

Longer-horizon opportunities that build institutional credibility:

### Residencies & Fellowships

| Opportunity | Amount | Timeline | Fit | Notes |
|-------------|--------|----------|-----|-------|
| **MacDowell** | Fully funded + stipends | Aug 15-Sep 10 | **9/10** | 10 days-6 weeks, NH |
| **Yaddo** | Room/board/studio | Aug 1 | 8/10 | 2 weeks-2 months, Saratoga Springs |
| **Recurse Center** | FREE (no cost) | Rolling | **9/10** | 6-12 weeks, Brooklyn |
| **SFPC** | Pay-what-you-can | Next session TBA | **9/10** | NYC, poetic computation |
| **Harvestworks New Works** | $5,000 + lab access | ~Dec 2026 | 8/10 | AR/VR, ML, AI, code-based media |
| **LMCC Workspace** | 9-month studio | Spring/Summer TBA | 8/10 | Manhattan, digital media |
| **NEW INC (New Museum)** | ~$600/mo membership | Spring/Summer 2026 app | 7/10 | 13th Cohort — tech-focused incubator |
| **Pioneer Works** | $1K–$2.5K + studio | Summer 2026 for 2027 | 7/10 | Brooklyn-based |
| **Stochastic Labs** | Fully funded + housing | Rolling | 7/10 | Berkeley, CA |
| **Ucross** | $1,500 + full room/board | Jul 15 | 7/10 | 2-4 weeks, Wyoming |
| **Tulsa Artist Fellowship** | **$230K+** ($150K + housing + healthcare) | Spring 2026 app for 2027-2029 | 7/10 | Requires relocation to Tulsa |
| **Queer\|Art Mentorship** | Program-based | ~Jun-Jul 2026 app | 8/10 | LGBTQ+ artists |
| **Lambda Literary Writers Retreat** | Program-based | ~Nov 2026-Jan 2027 | 7/10 | MFA-relevant writing focus |
| **NYSCA/NYFA Artist Fellowship** | $8,000 unrestricted | ~May 2026 for FY2027 | **9/10** | Requires fiscal sponsor |

### Major Grants (Summer-Fall)

| Opportunity | Amount | Timeline | Fit | Notes |
|-------------|--------|----------|-----|-------|
| **Guggenheim Fellowship** | $50K-$75K | Sep 2026 | 8/10 | Aspirational but genuine |
| **USA Fellowship** | $50,000 unrestricted | Watch for 2027 | 8/10 | Media or design category |
| **DCLA PAIR** | $40,000 | Spring 2026 TBA | 8/10 | Highest-value NYC individual grant |
| **Jerome Hill Fellowship** | Multi-year substantial | Watch for next cycle | 8/10 | Technology-centered arts, NYC |
| **Rhizome Commissions** | $2,500-$2,900 | Annual cycle TBA | 8/10 | Born-digital art |

### LGBTQ+-Specific

| Opportunity | Amount | Notes |
|-------------|--------|-------|
| **Queer\|Art Robert Giard Grant** | $10,000 | Emerging LGBTQ+ photographers — verify medium eligibility |
| **TJFP Cycles 2 & 3** | Unrestricted | Published 2026 dates |
| **Founders First "National Pride Grant"** | $1,000 | Pre-register; full app Apr or Sep 2026 |

### International (if applicable)

| Opportunity | Amount | Notes |
|-------------|--------|-------|
| S+T+ARTS Prize | EUR 20,000 | Opens ~Jan annually |
| Prix Ars Electronica | EUR 10,000+ | Early year deadline |
| Thoughtworks Arts | $10K + tech support | Theme-based open calls |

---

## Income Tracks — Beyond Grants

### Consulting ($100–125/hr minimum)

The $60/hr floor from early planning is **significantly below market** (FILE-058). AI orchestration consulting starts at $100–150/hr. Project-based pricing is strategically superior:

| Service | Price Range | Notes |
|---------|------------|-------|
| AI Readiness Audit | $2,500–$5,000 | Entry point, 40-60% convert to paid work |
| AI Integration Sprint | $5,000–$15,000 | Core offering |
| Monthly AI Advisory | $3,000–$7,500 | Retainer model |
| Custom AI System Build | $10,000–$50,000+ | The 150-repo system IS the credential |
| Workshop Facilitation | $500–$2,000/session | Teaching experience as differentiator |
| Documentation Consulting | $2,500–$15,000/project | ~6K+ words proves capacity |

**Benefits cliff note:** Consulting income is earned income (not lump-sum) — counts toward SNAP/Medicaid thresholds. Keep monthly consulting under $1,500 to stay below SNAP earned-income threshold ($23,484/yr). Or accept the Medicaid→Essential Plan transition and earn freely up to $39,125.

### Selective Employment (Only High-Fit Roles)

| Role | Fit | Salary | Status |
|------|-----|--------|--------|
| Together AI Lead DX | 6/10 | $160K–$240K | Best employment target — documentation + teaching angle |
| HuggingFace Dev Advocate | 5/10 | $120K–$160K | Skills-based, no years requirement |
| Anthropic FDE | 4/10 | $280K–$400K | Teaching-as-customer-facing — competitive reach |
| OpenAI SE Evals | 4/10 | $230K–$385K | Independent SWE + eval infrastructure |

**Below the line** (apply only if time permits): Cohere (3/10), Runway (3/10), Anthropic SE Claude Code (3/10).

**Benefits cliff note:** Full-time employment income blows through all thresholds. This is a total exit from the benefits safety net — which is fine IF the salary is sufficient ($120K+ provides a wide margin). Do NOT accept a $60K role that loses benefits worth $8K–$15K/yr.

---

## Writing Income — Premium Publications

A major discovery from research: several publications pay professional rates for exactly the kind of essays already being written. This creates a new income channel that leverages existing output.

| Publication | Rate | Topic Fit | Frequency |
|-------------|------|-----------|-----------|
| **Noema Magazine** | $1/word ($3K/3K-word essay) | Technology, governance, AI, culture | Quarterly |
| **MIT Technology Review** | $1-3/word ($2.5K-12K/feature) | AI, creative technology, LLMs | As pitched |
| **Wired** | $2,500+ starting | Technology stories | As pitched |
| **Logic Magazine** | $1,200-2,000/essay | Technology criticism | Themed issues |
| **Aeon** | $500-1,500 | Philosophy, culture, technology | As pitched |
| **Gay & Lesbian Review** | $250/feature | LGBTQ+ culture, arts | Ongoing |
| **e-flux journal** | ~$200-500 | Contemporary art theory | Prestige value |

**Benefits cliff note:** Writing income is **earned income** (not lump-sum) — counts toward SNAP monthly threshold ($1,580/month net for 1 person). One Noema essay per quarter ($3K/quarter = $12K/year) exceeds annual SNAP threshold but keeps within Essential Plan. Two essays per year ($6K) is safest on $12K base.

**Strategy:** Pitch Noema and Logic Magazine this month. Build toward 2-4 premium essay placements per year ($6K-$12K). This supplements grant income without the application overhead.

---

## Monetization Infrastructure

Set up passive income channels alongside active applications:

| Platform | Fee | Priority | Income Potential |
|----------|-----|----------|-----------------|
| **GitHub Sponsors** | 0% | **IMMEDIATE** | $50-500/mo |
| **Ko-fi** | 0% tips | **IMMEDIATE** | $50-200/mo |
| **Substack** | 10% + processing | This week | $300-3,000/mo |
| **Lemon Squeezy** | 5% + $0.50 | This month | Variable |

**Digital products to create (Month 2-3):** Governance Toolkit ($29-$49), Essay Collection PDF ($9.99), AI-Conductor Methodology Guide ($19-$39), README-as-Portfolio Template Pack ($9.99).

---

## Recovery-Informed Staging

From FILE-060 clinical research: cognitive recovery after combined trauma follows a predictable curve.

### Months 1–3: Stabilization (NOW)

- **Focus:** Low-barrier submissions, infrastructure activation, benefits compliance
- **Actions:** GitHub Sponsors, Fractured Atlas, Artadia, NEH, Creative Fellowship
- **Capacity:** Reduced executive function — keep tasks concrete and time-bound
- **Rule:** No new named internal sprints (P0 hermetic seal)

### Months 3–6: Light Production

- **Focus:** Targeted applications, first consulting engagement, portfolio refinement
- **Actions:** Creative Capital, Fire Island, Spencer, Processing Foundation
- **Capacity:** Improving focus — can handle multi-day application processes
- **Rule:** One major submission per week maximum

### Months 6–12: Full Execution

- **Focus:** Career-building residencies, full consulting practice, strategic employment
- **Actions:** Tulsa, NEW INC, Pioneer Works, Queer|Art, conference submissions
- **Capacity:** Professional-quality sustained work
- **Rule:** Reassess benefits cliff position quarterly

---

## Decision Framework

When evaluating any new opportunity:

1. **Fit score ≥ 5/10?** If not, skip unless zero-effort (rolling applications).
2. **Benefits cliff impact?** Check against thresholds above. Lump-sum grants = SNAP safe. Recurring stipends = count as income.
3. **Recovery-stage appropriate?** Don't attempt 6-hour applications during stabilization phase.
4. **Identity position match?** Use the framing that matches the funder (educator / artist / architect / community).
5. **Deadline within capacity?** Only commit to deadlines you can actually meet.

---

## Sources

- FILE-059: NYC Benefits Cliff Mechanics and Optimal Grant-Stacking
- FILE-060: Six Priorities for Building Professional Autonomy from Crisis
- FILE-058: February 2026 Job Market Intelligence: Three Career Lanes
- FILE-016/017: Comprehensive Funding Review and Grant-Finding Blueprint
- FUNDING_ACTION_SEQUENCE.md: Verified deadlines and amounts (2026-02-16)
- Web verification: Artadia, Creative Capital, Google Creative Fellowship deadlines confirmed 2026-02-17
